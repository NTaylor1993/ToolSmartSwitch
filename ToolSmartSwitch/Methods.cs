    ...
    public partial class ModEntry
    {
        ...
        public static void SmartSwitch(Farmer f)
        {
            ...
            if (Config.SwitchForResourceClumps)
            {
                ...
                {
                    foreach (ResourceClump clump in (f.currentLocation as Woods).resourceClumps)
                    {
                        var bb = clump.getBoundingBox();
                        if (bb.Intersects(tileRect))
                        {
                ...
                // TODO Figure out log
                if (f.currentLocation is Forest && (Game1.currentLocation as Forest).obsolete_log?.occupiesTile((int)tile.X, (int)tile.Y) == true)
                {
                    if (SwitchForClump(f, (Game1.currentLocation as Forest).obsolete_log, tools))
                        return;
                }
            }
            if (Config.SwitchForPan)
            {
                ...
                if (orePanRect.Contains((int)tile.X * 64, (int)tile.Y * 64) && Utility.distance((float)f.StandingPixel.X, (float)orePanRect.Center.X, (float)f.StandingPixel.Y, (float)orePanRect.Center.Y) <= 192f)
                {
            ...
            if (Config.SwitchForWateringCan)
            {
                ...
                // TODO : Check if bowl is watered already 
                ...
                if (f.currentLocation is Farm && f.currentLocation.getTileIndexAt((int)tile.X, (int)tile.Y, "Buildings") == 1938)
                {
                    if (SwitchToolType(f, typeof(WateringCan), tools))
                        return;
                }
            ...
            if (Config.SwitchForTilling && f.currentLocation.doesTileHaveProperty((int)tile.X, (int)tile.Y, "Diggable", "Back") != null && !f.currentLocation.IsTileOccupiedBy(new Vector2(tile.X, tile.Y)) && f.currentLocation.isTilePassable(new Location((int)tile.X, (int)tile.Y), Game1.viewport))
            {
                if (SwitchToolType(f, typeof(Hoe), tools))
                    return;
